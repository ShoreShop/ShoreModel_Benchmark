...
if __name__ == "__main__":
    ...
    time_scales = ['short', 'medium']
    preds = dict(zip(time_scales, [[], []]))
    ...
    start_time_preds = {
        ...
        'long': shorelines_obs.index[~shorelines_obs.isna().any(axis=1)][-1] # Date for last obs without nan
        }
    end_time_preds = {
        'short': datetime.datetime.strptime(END_TIME_SHORT_PRED, '%Y-%m-%d'), 
        'medium': datetime.datetime.strptime(END_TIME_MEDIUM_PRED, '%Y-%m-%d'), 
        'long': datetime.datetime.strptime(END_TIME_LONG_PRED, '%Y-%m-%d')
        }
    ...
    refs = {
        ...
        'long': shorelines_obs[~shorelines_obs.isna().any(axis=1)].iloc[-1] # Last obs without nan
        }
    ...
    for i, tran_id in enumerate(shorelines_obs.columns):
        ...
        for time_scale in time_scales:
            ...
            pred = pred - pred.iloc[0] + refs[time_scale][tran_id]
